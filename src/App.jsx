...
// Snapshot: real 48-team data captured from football-data.org (Jun 24 2026). Auto-refreshes on load.
const SEED_AT = "2026-06-24T18:07:13Z";
...
function enrich(row){ const p=row.w+row.d+row.l, pts=row.w*3+row.d, gd=row.gf-row.ga;
  return {...row, p, pts, gd, name:row.name||NAME[row.team]||row.team, g:row.group, t:row.team}; }
...
function thirdsFromStandings(st){
  ...
  Object.keys(byGroup).sort().forEach(g=>{
    ...
    // Live data carries FIFA-correct positions from the worker (within-group order
    // per Art. 13, head-to-head first on equal points). The offline snapshot has no
    // positions, so fall back to points -> GD -> goals (no head-to-head).
    const hasPos = teams.every(t=>Number.isFinite(t.pos));
    const sorted = hasPos
...
// ===== Live data =====
// Same-origin API served by this app's own Cloudflare Worker (see worker/index.js).
// Same origin => no CORS, and the football-data.org key stays a server-side secret.
const PROXY_URL = "/api/standings";
...
export default function App(){
  ...
  const srcBadge = {
    proxy:{label:"Live · football-data.org", cls:"bg-emerald-100 text-emerald-800", Icon:Wifi},
